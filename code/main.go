...
func signup(c *fiber.Ctx) error {
    ...
    return c.JSON(fiber.Map{"message": "User signed up successfully"})
}
...
func login(c *fiber.Ctx) error {
	...
	var user Credentials
	if err := db.DB.Where("user = ?", creds.User).First(&user).Error; err != nil {
    ...
    accessToken, err := generateAccessToken(user.User)
    if err != nil {
        return c.SendStatus(fiber.StatusInternalServerError)
    }

    // Generate refresh token
    refreshToken, err := generateRefreshToken(user.User)
    if err != nil {
        return c.SendStatus(fiber.StatusInternalServerError)
    }
